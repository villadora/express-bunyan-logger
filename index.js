...
module.exports.errorLogger = function (opts) {
  ...
  opts.hasOwnProperty("parseUA") &&
    ((parseUA = opts.parseUA), delete opts.parseUA);
  ...
  return function (err, req, res, next) {
    ...
    if (genReqId) requestId = genReqId(req);

    var childLogger =
      requestId !== undefined ? logger.child({ req_id: requestId }) : logger;
    req.log = childLogger;
    ...
    function logging(incoming) {
      ...
      var status = res.statusCode,
        ...
        ua = parseUA
          ? useragent.parse(req.header("user-agent"))
      ...
      if (excludes) {
        ...
        if (!~excludes.indexOf("*")) {
          ...
          for (var p in meta) if (!exs[p]) json[p] = meta[p];
        }
      }
